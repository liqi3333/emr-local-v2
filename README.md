# 电子病历系统 v2 — 本地部署版
1. **四种病历类型**：支持首次病程录、主治医师查房、主任医师查房、术前小结
2. **AI模型接入**：支持OpenAI、Claude、Gemini等主流AI模型配置
3. **智能生成**：通过AI对话或自然语言描述自动生成结构化病历
4. **患者管理**：Excel风格的患者列表，支持搜索、排序、分页
5. **数据存储**：SQLite后端 + IndexedDB离线存储，自动回退
## 项目架构
